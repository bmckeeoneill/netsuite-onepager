...
# ── Email function ──
def send_email(rep_email, rep_name, company_name, pdf_bytes, email_draft):
    import smtplib
    ...
    msg = MIMEMultipart()
    msg["From"] = "NetSuite One-Pager Generator <" + gmail_address + ">"
    msg["To"] = rep_email
    msg["Subject"] = "One-Pager Ready: " + company_name

    first_name = rep_name.split(",")[1].strip() if "," in rep_name else rep_name
    body = "Hi " + first_name + ",\n\nYour " + company_name + " one-pager is attached.\n\n--- SUGGESTED EMAIL DRAFT ---\n\n" + email_draft + "\n\n--- END DRAFT ---\n\nGenerated by NetSuite One-Pager Generator"
    msg.attach(MIMEText(body, "plain"))

    att = MIMEBase("application", "octet-stream")
    att.set_payload(pdf_bytes)
    encoders.encode_base64(att)
    att.add_header("Content-Disposition", "attachment; filename=" + company_name + "_onepager.pdf")
    msg.attach(att)

    with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
        server.login(gmail_address, gmail_password)
        server.sendmail(gmail_address, rep_email, msg.as_string())
...
if submitted:
    ...
    rep_data = get_rep(rep_name)
    rep_email = rep_data.get("email", "")
    email_draft = content.get("email_draft", "No draft available.")
    if rep_email:
        try:
            send_email(rep_email, rep_name, company_name, pdf_bytes, email_draft)
            st.info("📧 Emailed to " + rep_email)
        except Exception as e:
            st.warning("PDF ready but email failed: " + str(e))
